...
from arxiv_auth.legacy import accounts, sessions as legacy_sessions
...
def register(method: str, params: MultiDict, ip: str,
    ...
    if method == 'GET':
        ...
        try:
            user, auth = accounts.register(form.to_domain(), password or "", ip, ip)
...
class ProfileForm(Form):
    ...
    GROUPS = [
        ...
        if group and not group.is_test
    ]
    CATEGORIES = [
        (archive.id, [
...
def validate_username(data) -> None:
    """Ensure that the username is unique."""
    if accounts.does_username_exist(data):
...
def validate_email(data) -> None:
    """Ensure that the email address is unique."""
    if accounts.does_email_exist(data):
        raise ValidationError(Markup(
            'An account with that email already exists. You can try'
        ))
