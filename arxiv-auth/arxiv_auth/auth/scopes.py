...
def scope_str(domain:str, action:str, resource:str = None):
...
READ_PUBLIC = scope_str(domains.PUBLIC, actions.READ)
"""
...
EDIT_PROFILE = scope_str(domains.PROFILE, actions.UPDATE)
"""
...
VIEW_PROFILE = scope_str(domains.PROFILE, actions.READ)
"""
...
CREATE_SUBMISSION = scope_str(domains.SUBMISSION, actions.CREATE)
"""Authorizes creating a new submission."""

EDIT_SUBMISSION = scope_str(domains.SUBMISSION, actions.UPDATE)
"""Authorizes updating a submission that has not yet been announced."""

VIEW_SUBMISSION = scope_str(domains.SUBMISSION, actions.READ)
"""Authorizes viewing a submission."""

DELETE_SUBMISSION = scope_str(domains.SUBMISSION, actions.DELETE)
"""Authorizes deleting a submission."""

PROXY_SUBMISSION = scope_str(domains.SUBMISSION, actions.PROXY)
"""
...
READ_UPLOAD = scope_str(domains.UPLOAD, actions.READ)
"""Authorizes viewing the content of an upload workspace."""

WRITE_UPLOAD = scope_str(domains.UPLOAD, actions.UPDATE)
"""Authorizes uploading files to to a workspace."""

RELEASE_UPLOAD = scope_str(domains.UPLOAD, actions.RELEASE)
"""Authorizes releasing an upload workspace."""

DELETE_UPLOAD_WORKSPACE = scope_str(domains.UPLOAD, 'delete_workspace')
"""Can delete an entire workspace in the file management service."""

DELETE_UPLOAD_FILE = scope_str(domains.UPLOAD, actions.DELETE)
"""Can delete files from a file management upload workspace."""

READ_UPLOAD_LOGS = scope_str(domains.UPLOAD, 'read_logs')
"""Can read logs for a file management upload workspace."""

READ_UPLOAD_SERVICE_LOGS = scope_str(domains.UPLOAD, 'read_service_logs')
"""Can read service logs in the file management service."""

CREATE_UPLOAD_CHECKPOINT = scope_str(domains.UPLOAD, 'create_checkpoint')
"""Create an upload workspace checkpoint."""

DELETE_UPLOAD_CHECKPOINT = scope_str(domains.UPLOAD, 'delete_checkpoint')
"""Delete an upload workspace checkpoint."""

READ_UPLOAD_CHECKPOINT = scope_str(domains.UPLOAD, 'read_checkpoints')
"""Read from an upload workspace checkpoint."""

RESTORE_UPLOAD_CHECKPOINT = scope_str(domains.UPLOAD, 'restore_checkpoint')
"""Restore an upload workspace to a checkpoint."""

READ_COMPILE = scope_str(domains.COMPILE, actions.READ)
"""Can read documents generated by the compilation service."""

CREATE_COMPILE = scope_str(domains.COMPILE, actions.CREATE)
"""Can create new documents via the compilation service."""

READ_FULLTEXT = scope_str(domains.FULLTEXT, actions.READ)
"""Can access plain text extracted from compiled documents."""

CREATE_FULLTEXT = scope_str(domains.FULLTEXT, actions.CREATE)
"""Can trigger new plain text extractions from compiled documents."""

READ_PREVIEW = scope_str(domains.PREVIEW, actions.READ)
"""Can view a submission preview."""

CREATE_PREVIEW = scope_str(domains.PREVIEW, actions.CREATE)
