...
def test_unprivileged_user(fastapi, secret):
    res = fastapi.get("/", cookies={'ARXIVNG_SESSION_ID': user_jwt(USER_ID_NO_PRIV, secret)})
    assert res.status_code == 401  

    res = fastapi.get("/", headers={'Authorization': 'Bearer ' + user_jwt(USER_ID_NO_PRIV, secret)})
    assert res.status_code == 401  
...
def test_mod(fastapi, secret):
    ...
    assert set(user['moderated_categories']) == set(['bicycles.chopped', 'bicycles.tall'])
