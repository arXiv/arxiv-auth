...
def save_client(
    ...
    """
    with util.transaction() as TapirSession:
        if client.client_id:
            db_client = _load_dbclient(client.client_id, TapirSession)
...
def set_credential(cred: domain.ClientCredential,
    ...
    with util.transaction(commit) as TapirSession:
        if db_client is None:
            db_client = _load_dbclient(client_id, TapirSession)
        if db_client.credential:
            db_client.credential.client_secret = cred.client_secret
            TapirSession.add(db_client)
        else:
            TapirSession.add(models.DBClientCredential(
                client=db_client,
...
def update_authorizations(auths: List[domain.ClientAuthorization],
    ...
    with util.transaction(commit) as TapirSession:
        ...
        for auth_id in set(extant_auths.keys()) - auths_to_keep:
            TapirSession.delete(extant_auths[auth_id])
...
def update_grant_types(grant_types: List[domain.ClientGrantType],
    ...
    with util.transaction(commit) as TapirSession:
        ...
        for gtype_id in set(extant_gtypes.keys()) - gtypes_to_keep:
            TapirSession.delete(extant_gtypes[gtype_id])
        ...
        for gtype in grant_types:
            ...
            # Update an existing auth.
                ...
                TapirSession.add(db_grant_type)
            else:
...
def save_auth_code(code: domain.AuthorizationCode) -> None:
    ...
    with util.transaction() as TapirSession:
        ...
        TapirSession.add(db_code)


def delete_auth_code(code: str, client_id: int) -> None:
    """Delete an auth code from the database."""
    with util.transaction() as TapirSession:
        db_code = _load_dbauthcode(code, client_id, TapirSession)
        TapirSession.delete(db_code)
...
def load_auth_code_by_user(code: str, user_id: str) \
    ...
    """Load an authorization code for an API client."""
    with util.transaction() as TapirSession:
        db_code = TapirSession.query(models.DBAuthorizationCode)\
...
def _load_dbauthcode(code: str, client_id: int, TapirSession: util.Session) \
        -> None:
    db_code = TapirSession.query(models.DBAuthorizationCode)\
        .filter(models.DBAuthorizationCode.code == code) \
...
def _load_dbclient(client_id: str, TapirSession: util.Session) -> models.DBClient:
    db_client: models.DBClient = TapirSession.query(models.DBClient) \
